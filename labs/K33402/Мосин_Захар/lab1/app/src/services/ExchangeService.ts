...
class ExchangeService {
	public static async getUserExchanges(senderId: number) {
		try {
			return await exchangeRepository.findAll({ where: { senderId } })
		} catch (error) {
		...
		public static async getAllExchanges() {
		try {
			return await exchangeRepository.findAll()
		} catch (error) {
			return serviceHandleError({
				error,
				message: 'Ошибка при получении обменов',
			})
		}
	}
